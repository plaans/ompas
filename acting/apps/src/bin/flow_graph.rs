...
use ompas_core::planning::conversion::debug_with_markdown;
...
use ompas_core::planning::conversion::{_convert, TEST_CONVERSION};
...
Graph flow converter for SOMPAS code!\n
    ...
    {
        println!("Please run binary with feature \"conversion_data\".");
        exit(0)
    }
    #[allow(unreachable_code)]
    {
        ...
        for p in &config.problems {
            ...
            {
                let _lv = parse(&problem, &mut env)
                    .await
                    .unwrap_or_else(|r| panic!("{}", r.to_string()));
                let _ch: ActingModel = crate::__convert(
                    _lv,
                    PLEnv {
                        env,
