use crate::planning::conversion::point_algebra::problem::{PAGraph, Timepoint};
use crate::planning::conversion::point_algebra::relation_type::RelationType;
...
pub fn path_consistency<T>(mut m: PAGraph<T>) -> lruntimeerror::Result<PAGraph<T>> {
    // Q := {(i,j ) | i<j }
...
pub fn revise<T>(m: &mut PAGraph<T>, i: Timepoint, j: Timepoint, k: Timepoint) -> bool {
    let r1 = &m[(i, k)];
    let r2 = &m[(k, j)];
...
pub fn remove_useless_timepoints<T>(mut m: PAGraph<T>) -> lruntimeerror::Result<PAGraph<T>> {
