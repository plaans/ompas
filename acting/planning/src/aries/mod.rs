...
use ompas_structs::sym_table::domain::{cst, Domain, TypeId};
...
pub fn generate_templates(problem: &PlanningProblem) -> anyhow::Result<chronicles::Problem> {
    ...
    for command in &domain.commands {
        let mut template =
            read_chronicle(&mut ctx, &mut bindings, &command.template, cont.clone())?;
        template.chronicle.kind = ChronicleKind::Action;
        Printer::print_chronicle(&template.chronicle, &ctx.model);
        templates.push(template);
    }

    for method in &domain.methods {
        let mut template = read_chronicle(&mut ctx, &mut bindings, &method.template, cont.clone())?;
        template.chronicle.kind = ChronicleKind::Method;
        Printer::print_chronicle(&template.chronicle, &ctx.model);
...
fn atom_from_cst(ctx: &Ctx, cst: &cst::Cst) -> aAtom {
    match cst {
...
pub fn create_initial_chronicle(instance: &PlanningInstance, ctx: &mut Ctx) -> ChronicleInstance {
    ...
    initialize_state(&mut init_ch, &instance.state, ctx);
...
fn convert_constraint(
    ...
    match c {
        ...
        | Constraint::Or(_)
        | Constraint::Type(_, _)
        | Constraint::Arbitrary(_) => Err(LRuntimeError::default().chain("constraint::arbitrary"))?,
        Constraint::Neq(a, b) => {
            let a: VarId = a.try_into()?;
            match b {
                Lit::Atom(b) => {
...
pub fn generate_chronicles(problem: &PlanningProblem) -> Result<chronicles::Problem> {
    ...
    let init_ch = create_initial_chronicle(&problem.instance, &mut p.context);
