use ompas_lisp::structs::{LValue, LError, NameTypeLValue, GetModule, Module};
use ompas_lisp::structs::LValue::Nil;
use crate::rae::context::{ActionsProgress, Agenda, RAEOptions, RAEEnv, Status, ActionId, SelectOption};
...
pub const LAMBDA_DEF_METHOD: &str = "(defmacto defmethod ...)";
pub const DEF_METHOD: &str = "defmethod";

pub const LAMBDA_DEF_ACTION: &str ="(defmacto defaction ...)";
...
pub fn add_action(args: &[LValue], _env: &mut LEnv, ctx: &mut CtxRAE) -> Result<LValue, LError> {
    ...
    if let LValue::Symbol(action_label) = &args[0] {
        if let LValue::Lambda(body) = &args[1] {
            ...
            return Err(WrongType(args[1].clone(), args[1].clone().into(), NameTypeLValue::Lambda))
        }
    }else {
        return Err(WrongType(args[0].clone(), args[0].clone().into(), NameTypeLValue::Symbol))
    }
    ...
    if let LValue::Symbol(method_label) = &args[0] {
        if let LValue::Symbol(task_label) = &args[1] {
            if let LValue::Lambda(body) = &args[2] {
                ctx.env.add_method(method_label.to_string(), &task_label.to_string(), body.clone());
            } else {
                return Err(WrongType(args[2].clone(), args[2].clone().into(), NameTypeLValue::Lambda))
            }
        }else {
            return Err(WrongType(args[1].clone(), args[1].clone().into(), NameTypeLValue::Symbol))
        }
    }else {
        return Err(WrongType(args[0].clone(), args[0].clone().into(), NameTypeLValue::Symbol))
    }
...
///Add a task to RAE env
pub fn set_exec_command(_: &[LValue], _env: &mut LEnv, _: &mut CtxRAE) -> Result<LValue, LError> {
    todo!()
}

pub fn get_exec_command(_: &[LValue], _env: &LEnv, _: &CtxRAE) -> Result<LValue, LError> {
    todo!()
}
...
pub fn rae_get_env(args: &[LValue], _env: &LEnv, ctx: &CtxRAE) -> Result<LValue, LError> {
    let key = match args.len() {
        0 => None,
        1 => if let LValue::Symbol(key) = args[0].clone() {
            Some(key)
        }else {
            return Err(WrongType(args[0].clone(), args[0].clone().into(), NameTypeLValue::Symbol))
        }
