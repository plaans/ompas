# Table of contents

- [Operational Model Planning and Acting System (OMPAS)](ompas.md)
- [How to run the system](run.md)

## OMPAS

- [Acting System](system/intro.md)
  - [Interacting with the REPL](system/repl.md)
  - [Configuring OMPAS](system/configuration.md)
- [Acting Language](language/al.md)
  - [Scheme](language/scheme/intro.md)
    - [Modules](language/scheme/modules/intro.md)
      - [Module IO](language/scheme/modules/module-io.md)
      - [Module String](language/scheme/modules/module-string.md)
      - [Module Utils](language/scheme/modules/module-utils.md)
      - [Module Advanced math](language/scheme/modules/module-advanced-math.md)
  - [Acting primitives](language/primitives.md)
  - [Resource management](language/resources.md)
  - [Advanced acting functions](language/advanced.md)
  - [Acting domain definition](language/domain.md)
  


## Platform integration

- [Gobot-Sim](gobot-sim/intro.md)


## Implementation

<!--- 
- [Refinement Acting Engine (RAE)](rae/def.md)
    - [Algorithms](rae/algorithms.md)
    - [Define a Platform in RAE](rae/platform-definition.md)
- [The scheme interpreter](sompas/interpreter.md)
- [Implementation of a sompas-like language in Rust](sompas/implementation/README.md)
  - [LValue](sompas/implementation/lvalue/README.md)
    - [LValueS](sompas/implementation/lvalue/lvalues.md)
  - [LError](sompas/implementation/lerror.md)
  - [Environment](sompas/implementation/environment.md)
  - [Functions](sompas/implementation/functions/README.md)
    - [Core Operator](sompas/implementation/functions/core-operator.md)
    - [Lambda](sompas/implementation/functions/lambda.md)
    
- [Create binaries with the interpreter.](sompas/binaries.md)
- [The REPL](sompas/repl.md)
- [How to run the Scheme interpreter](sompas/run.md)
- [The task handler](sompas/taskhandler.md)
-->

