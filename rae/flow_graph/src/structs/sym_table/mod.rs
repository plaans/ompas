...
impl SymTable {
    ...
    pub fn new_nil(&mut self) -> VarId {
        self.new_variable(NIL, RootType::Nil)
    }
    ...
    pub fn new_if(&mut self) -> (VarId, VarId, VarId) {
        ...
        let id_if = self.new_variable(sym_if, self.get_type_as_domain(TASK_TYPE));
        self.ids.insert(&sym_if, &id_if);

        let sym_m_true = &format!("m_{}_true", sym_if);
        let id_m_true = self.new_variable(sym_m_true, self.get_type_as_domain(METHOD_TYPE));
        self.ids.insert(&sym_m_true, &id_m_true);

        let sym_m_false = &format!("m_{}_false", sym_if);
        let id_m_false = self.new_variable(sym_m_true, self.get_type_as_domain(METHOD_TYPE));
