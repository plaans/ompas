...
use ompas_rae_structs::planning::atom::Sym;
use ompas_rae_structs::planning::chronicle::ChronicleTemplate;
use ompas_rae_structs::planning::condition::Condition;
use ompas_rae_structs::planning::constraint::*;
use ompas_rae_structs::planning::effect::Effect;
use ompas_rae_structs::planning::expression::Expression;
use ompas_rae_structs::planning::expression_chronicle::ExpressionChronicle;
use ompas_rae_structs::planning::interval::Interval;
use ompas_rae_structs::planning::lit::{lvalue_to_lit, Lit};
use ompas_rae_structs::planning::symbol_table::{AtomId, ExpressionType};
use ompas_rae_structs::planning::traits::{Absorb, FormatWithSymTable, GetVariables};
use ompas_rae_structs::planning::type_table::{AtomKind, PlanningAtomType, VariableKind};
use ompas_rae_structs::planning::{ConversionCollection, ConversionContext, TaskType, COND};
                                ...
                                match l.len() {
                                    2 => {
                                        ...
                                        if symbol_type.is_result_pure() {
                                            ...
                                            return Err(Anyhow(CONVERT_LVALUE_TO_EXPRESSION_CHRONICLE, "cannot handle (instance <type>) with <type> undecided".to_string()));
                                        }
                                    }
                                    3 => {
                            ...
                            _ => {
                                if let Some(id) = ch.sym_table.id(&s) {
                                        ...
                                        .expect("a defined symbol should have a type").a_type.unwrap()
                                    {
                                        ...
                                        PlanningAtomType::Method => return Err(Anyhow(CONVERT_LVALUE_TO_EXPRESSION_CHRONICLE, format!("{} is method and can not be directly called into the body of a method.\
                                \nPlease call the task that use the method instead", s))),
                                        ...
                                        _ => return Err(Anyhow(CONVERT_LVALUE_TO_EXPRESSION_CHRONICLE, format!("{}: first symbol should be a function, task, action or state function", s))),
                                    }
