...
use crate::context::rae_env::{Action, RAEEnv, StateFunction};
use crate::context::rae_state::*;
use crate::module::mod_rae_description::*;
...
const RAE_CONVERT_DOMAIN: &str = "convert-expr";
const RAE_PRE_PROCESS_LAMBDA: &str = "convert-expr";
const RAE_PRE_PROCESS_EXPR: &str = "convert-expr";
const RAE_PRE_PROCESS_DOMAIN: &str = "convert-expr";
const RAE_CONVERT_COND_EXPR: &str = "convert-expr";
...
async fn convert_expr<'a>(args: &'a [LValue], env: &'a LEnv) -> LResult {
    ...
    let ctx = env.get_context::<CtxRae>(MOD_RAE)?;
    let mut rae_env: RAEEnv = ctx.own_rae_env().await;

    let lv = expand(&args[0], true, &mut rae_env.env).await?;

    let mut symbol_table = SymTable::default();
    let time = SystemTime::now();
    let chronicle =
        translate_lvalue_to_expression_chronicle(&lv, &rae_env.into(), &mut symbol_table)?;
...
pub async fn convert_domain<'a>(_: &'a [LValue], env: &'a LEnv) -> LResult {
    ...
    let time = SystemTime::now();
    let (domain, st) = translate_domain_env_to_hierarchy(rae_env.into())?;
    let time = time.elapsed().expect("could not get time").as_micros();
...
pub async fn pre_process_lambda<'a>(args: &'a [LValue], env: &'a LEnv) -> LResult {
    ...
    transform_lambda_expression(&args[0], rae_env.env)
}
