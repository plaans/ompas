    ...
    match values.len() {
        2 => {
            ...
            for (i,val) in values.iter().enumerate() {
                match val {
                    LispValue::Atom(LispAtom::Number(LispNumber::Int(int)))=> {
                        match i {
                    ...
                    LispValue::Atom(LispAtom::Number(LispNumber::Float(float))) => {
                        match i {
                            0 => first_val = *float,
                            1 => second_val = *float,
                            _ => panic!("Strong error")
                    ...
                    lv => return Err(LispError::WrongType(lv.clone().into(), NameTypeLispValue::NAtom))
                };
            }

            Ok(LispValue::Atom(LispAtom::Number(LispNumber::Float(first_val - second_val))))
        }
...
pub fn gt(values: Vec<LispValue>) -> Result<LispValue, LispError> {
    match values.len(){
        2 => {
            Ok(LispValue::Atom(LispAtom::Bool(values[0] > values[1])))
        },
        i => Err(WrongNumerOfArgument(i, 2..2))
    }
}
...
pub fn is_none(values: Vec<LispValue>) -> Result<LispValue, LispError> {
    match values.len() {
        1 => Ok(LispValue::Atom(LispAtom::Bool(NameTypeLispValue::from(values[0].clone()) == NameTypeLispValue::None))),
...
pub fn is_number(values: Vec<LispValue>) -> Result<LispValue, LispError> {
    match values.len() {
        1 => Ok(LispValue::Atom(LispAtom::Bool(NameTypeLispValue::from(values[0].clone()) == NameTypeLispValue::NAtom))),
        i => Err(WrongNumerOfArgument(i, 1..1))
    }    }

pub fn is_bool(values: Vec<LispValue>) -> Result<LispValue, LispError> {
    match values.len() {
        1 => Ok(LispValue::Atom(LispAtom::Bool(NameTypeLispValue::from(values[0].clone()) == NameTypeLispValue::BAtom))),
        i => Err(WrongNumerOfArgument(i, 1..1))
    }    }

pub fn is_fn(values: Vec<LispValue>) -> Result<LispValue, LispError> {
    match values.len() {
        1 => Ok(LispValue::Atom(LispAtom::Bool(NameTypeLispValue::from(values[0].clone()) == NameTypeLispValue::LispFn))),
        i => Err(WrongNumerOfArgument(i, 1..1))
    }    }
