...
pub const PLANNING_TIME: &str = "TP";
...
impl Field {
    pub fn to_latex(&self) -> String {
        match self {
            ...
            DeliberationTimeRatio => "$T_D/T_E$",
            Coverage => "\\textit{{Cov}}",
            EfficiencyScore => "$\\hat{E_S}$",
            BestScoreRatio => "$\\hat{RE_S^{*}}$",
            DistanceToBestScore => "$\\hat{DE^{*}_S}$",
            NumberRetries => "$N_R$",
            NumberFailures => "N_F",
            PlanningWaitingTime => "$T_{WP}$",
            PlanningWaitingTimeRatio => "$RT_{WP}$",
            PlanningTime => "$T_{P}$",
            PlanningTimeRatio => "$T_{P}/T_{E}$",
